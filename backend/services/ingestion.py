...
def detect_condition_category(text: str) -> str:
    text_lower = text.lower()
    categories = {
...
def update_corpus_manifest(title: str, source: str, file_path: str, chunks_created: int):
...
def ingest_document(db: Session, doc_id: uuid.UUID) -> int:
    ...
    db.commit()
    update_corpus_manifest(doc.title, doc.source, doc.file_path, len(chunk_records))
